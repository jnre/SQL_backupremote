...
/****** Object:  Job [daily backup script]    Script Date: 03/01/2020 4:11:56 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 03/01/2020 4:11:56 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'daily backup script', 
		@enabled=0, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'backs up specified full database 
@db0 = jojopower;
...
@daysToKeep = 2;', 
		...
		@owner_login_name=N'UNIVAC\mesops', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [check for disk space]    Script Date: 03/01/2020 4:11:56 PM ******/
...
if($report){
    ...
    Throw "$message";
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [attempt to backup daily]    Script Date: 03/01/2020 4:11:57 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'attempt to backup daily', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_success_step_id=0, 
...
SELECT @fileLocation= (SELECT @paramFile +@paramdb0+''full'' +REPLACE(CONVERT(VARCHAR(20),GETDATE(),103),''/'','''') + ''_'' + REPLACE(CONVERT(VARCHAR(20),GETDATE(),108),'':'','''') +''.bak'')
BACKUP DATABASE @paramdb0 to DISK =@fileLocation 
WITH NOFORMAT, 
           NOINIT,
           SKIP,
           REWIND,
           NOUNLOAD,
           STATS = 10', 
		@database_name=N'jojopower', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [transfer to remote and remove backup]    Script Date: 03/01/2020 4:11:57 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'transfer to remote and remove backup', 
		@step_id=3, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'PowerShell', 
		@command=N'#copy kepwareDB.bak files to remote server
...
$daysToKeep = $desc[0].SubString([int]$desc[0].IndexOf(''@daysToKeep'') + [int](''@daysToKeep = '').length,[int]$desc[0].IndexOf('';'',[int]$desc[0].IndexOf(''@daysToKeep'')) -[int]$desc[0].IndexOf(''@daysToKeep'') - [int](''@daysToKeep = '').length);

#remove first in remote before transfer
$DatetoDelete = (Get-Date).AddDays(-$daysToKeep).AddMinutes(-10);
try{
    $deleteOldDB = Get-ChildItem $remoteFileLocation -Recurse | Where-Object {$_.LastWriteTime -lt $DatetoDelete } | Remove-Item
}

catch{
    #remove old db failed
    $nocopyMessage = ConvertTo-Json @{text = "delete-Item backup of $daysToKeep day failed"};
...
#copy to remote  
    ...
    $copyValidate = Copy-Item -Path ($fileLocation + $db0 + "full*.bak") -PassThru -Destination $remoteFileLocation;
    if($copyValidate){
        $copyValidate;
        #delete in current folder
        #remove-Item -Path ($fileLocation + $db0 + "full*.bak") -Force;
    }
    else{
        #copy failed
        $nocopyMessage = ConvertTo-Json @{text = "copy-Item failed to transfer to backup"};
        Invoke-RestMethod -Method Post -ContentType ''application/json'' -Body $nocopyMessage -Uri $channel;
    }
}
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'daily backup', 
		...
		@schedule_uid=N'4773ab79-9daf-4795-abf8-fc7880f30dae'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
